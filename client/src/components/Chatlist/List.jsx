...
import { useFetchGroupQuery, useGetInitialContactQuery } from '../../redux/message/messageApi';
...
function List() {
    ...
    const { data: dataGroup } = useFetchGroupQuery();
    ...
    return (
        <div className="bg-transparent flex-auto overflow-auto max-h-full custom-scrollbar mt-6">
            {dataGroup?.group.map((group) => (
                <ChatListItem data={group} key={group.id} />
            ))}
